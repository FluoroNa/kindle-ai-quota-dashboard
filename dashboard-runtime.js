(function (win, doc) {
  ...
  function updateQuotaCard(cardId, source) {
    ...
    for (index = 0; index < rows.length; index += 1) {
      ...
      var tokenInfo = '';
      if (quotaWindow.usedTokens != null && quotaWindow.limitTokens != null) {
        tokenInfo = ' ' + formatTokens(quotaWindow.usedTokens) + '/' + formatTokens(quotaWindow.limitTokens);
      }
      ui.textNode(
        rows[index].querySelector('.q-refresh'),
        (quotaWindow.detailText || remainingTime(quotaWindow.resetAt)) + tokenInfo
      );
    }
  ...
  function toggleCard(cardId, source) {
    var card = ui.find(cardId);
    if (!card) return;
    ui.style(card, 'display', source && source.disabled ? 'none' : '');
  }
  ...
  function renderCalendar() {
    ...
    ui.text('calTitle', year + '年' + months[month]);
    ...
    for (var r = 0; r < rows; r++) {
      ...
      for (var c = 0; c < 7; c++) {
        ...
        if (cellCount < firstDay) {
          dayNum = prevDays - firstDay + cellCount + 1;
          cls = ' class="other"';
        } else if (cellCount >= totalCells) {
          dayNum = cellCount - totalCells + 1;
          cls = ' class="other"';
        } else {
          dayNum = cellCount - firstDay + 1;
          if (dayNum === today) cls = ' class="today"';
        }
        html += '<td' + cls + '>' + dayNum + '</td>';
        cellCount++;
      }
      html += '</tr>';
    }
    ui.html(ui.find('calBody'), html);
  }
  function present(data) {
    ...
    if (data.updatedAt !== state.renderedAt) {
      ...
      toggleCard('cardClaude', data.sources.claude);
      toggleCard('cardCodex', data.sources.codex);
      toggleCard('cardKimi', data.sources.kimi);
      toggleCard('cardDeepSeek', data.sources.deepseek);
