  ...
  return {
    ...
    sources: {
      ...
      kimi: {
        ...
        label: 'Kimi',
        windows: [
          { name: '5小时', usedPct: 35, usedTokens: 35000, limitTokens: 100000, resetAt: afterHours(4) },
          { name: '周', usedPct: 56, usedTokens: 560000, limitTokens: 1000000, resetAt: afterHours(72) },
        ],
        fetchedAt: now,
        error: null,
      },
  ...
  for (const name of SOURCE_NAMES) {
    ...
    if (name === 'deepseek') {
      if (!Object.prototype.hasOwnProperty.call(source, 'balance')) {
        throw new Error(name + ' 缺少 balance');
      }
